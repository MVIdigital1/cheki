...
export async function POST(req: NextRequest) {
  ...
  if (useCached) {
    ...
    // Сайт ОФД не отдал реальный чек (капча/антибот) — вернулась только страница-заглушка.
    // Это НЕ значит "товаров акции нет": мы вообще не увидели содержимое чека.
    // Возвращаем честную ошибку, а не молчаливое "0 позиций", чтобы промоутер увидел
    // кнопки "ввести вручную" / "сфотографировать чек" вместо обманчивого результата.
    if (!looksLikeRealContent(rawText)) {
      return NextResponse.json(
        {
          error:
            "Сайт ОФД не показал содержимое чека (похоже на капчу/защиту от ботов). Пройдите капчу вручную или сфотографируйте чек.",
        },
        { status: 502 }
      );
    }
